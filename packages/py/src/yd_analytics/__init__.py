...
__all__ = [
    "run_query", "run_graph", "profile", "make_router",
    ...
    "ProfileResult", "ProfiledColumn", "__version__",
]


def run_query(engine: Engine, query: MetricQuery, *, role: str = "*") -> PanelResponse:
    """Punto de entrada tabular: ejecuta la métrica y resuelve su gráfico."""
    spec = registry.get(query.metric)
    result = run(engine, query, role=role)
